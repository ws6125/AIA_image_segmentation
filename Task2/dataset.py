...
def getImg(img_path):
    ...
    if '.dcm' in img_path:
        ...
        # pixels = np.pad(pixels, ((size_width - pixels.shape[0]) // 2), imgPadding)
        ...
        # img = np.pad(img, ((size_width - img.shape[0]) // 2), imgPadding)
...
class FeatureDataset(Dataset):
    def __init__(self, only_mri = False):
        self.images = []
        self.labels = []

        if not only_mri:
            self.getAllDataPath(f'{path}/CHAOS_AIAdatasets/2_Domain_Adaptation_dataset/CT')
        self.getAllDataPath(f'{path}/CHAOS_AIAdatasets/2_Domain_Adaptation_dataset/MRI/MRI_Label')
...
class ValidDataset(Dataset):
    def __init__(self):
        ...
        self.getAllDataPath(f'{path}/CHAOS_AIAdatasets/2_Domain_Adaptation_dataset/MRI/MRI_nonLabel')
...
class TestDataset(Dataset):
    def __init__(self, output_ori_image = False):
        self.output_ori_image = output_ori_image
        self.images = []
        self.image_size = []

        self.getAllDataPath(f'{path}/CHAOS_AIAdatasets/2_Domain_Adaptation_dataset/testset')
    ...
    def getImg(self, img_path):
        ...
        if '.dcm' in img_path:
            ...
            if self.output_ori_image:
                ...
                output_path = output_path.replace('testset', 'Pred').replace('.dcm', '.png')
...
if '__main__' == __name__:
    ...
    print("Numbers: ", len(valid_dataset))
    valid_loader = DataLoader(dataset = valid_dataset, batch_size = 2, shuffle = True)
    # for image, path in valid_loader:
    #     print(image.shape)

    test_dataset = TestDataset()
    print("Numbers: ", len(test_dataset))
